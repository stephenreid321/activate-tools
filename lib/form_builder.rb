    ...
    module FormBuilder
      class ActivateFormBuilder < AbstractFormBuilder
        ...
        def lookup_block(fieldname, lookup_url: nil, placeholder: nil, selected_link: nil, required: false, disabled: false, tip: nil, hint: nil, container_class: 'form-group', label_class: nil, div_class: nil)
          content = hidden_field(fieldname, :class => "lookup #{'is-invalid' if !error_message_on(fieldname.to_s.gsub('_id','')).blank?}", :'data-lookup-url' => lookup_url, :placeholder => placeholder, :required => (r = required || model_required(fieldname)), :disabled => disabled)
